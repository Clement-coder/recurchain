...
describe("RecurChainAgent - Original TS Tests", function () {
  let recurChainAgent;
  let usdcToken;
  let owner;
  let addr1;
  ...
  describe("Agent Management", function () {
    ...
    it("Should allow updating an agent", async function () {
      ...
      await expect(
        ...
        .withArgs(1, owner.address, newName, newAmount, newFrequency);
  ...
  describe("Admin Functions", function () {
    it("addr1 should not be the owner", async function () {
      expect(await recurChainAgent.owner()).to.not.equal(addr1.address);
    });

    it("Should revert when non-owner calls pause()", async function () {
      await expect(recurChainAgent.connect(addr1).pause())
        .to.be.revertedWithCustomError(recurChainAgent, "OwnableUnauthorizedAccount");
    });

    it("Should allow owner to update platform fee", async function () {
      const newFee = 50; // 0.5%
      await recurChainAgent.updatePlatformFee(newFee);
    ...
    it("Should allow owner to update fee collector", async function () {
      ...
      await recurChainAgent.updateFeeCollector(newCollector);
    ...
    it("Should allow owner to pause and unpause contract", async function () {
      await recurChainAgent.pause();
      expect(await recurChainAgent.paused()).to.be.true;

      await recurChainAgent.unpause();
